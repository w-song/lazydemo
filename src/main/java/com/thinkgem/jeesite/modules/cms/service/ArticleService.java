...
public class ArticleService extends CrudService<ArticleDao, Article> {
    ...
    public Article get(String id) {
        ...
        article.setId(id);
        article = super.get(article);
        // 通过批量编号获取FTP对象
        if (StringUtils.isNotBlank(article.getImage())) {
            List<FtpFile> ftpFileList = ftpFileService.findList(article.getImage());
            article.setImageFtpFile(ftpFileList.isEmpty() ? null : ftpFileList.get(0));
        }
        return article;
    }
