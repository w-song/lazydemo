...
public interface CategoryDao extends TreeDao<Category> {
    ...
    // {
    // return find("from Category where parentIds like :p1", new
    // Parameter(parentIds));
    // }
    ...
    // {
    // return
    // find("from Category where delFlag=:p1 and (module='' or module=:p2) order by site.id, sort",
    ...
    // {
    // return
    // find("from Category where delFlag=:p1 and parent.id=:p2 and inMenu=:p3 order by site.id, sort",
    ...
    // {
    // return
    // find("from Category where delFlag=:p1 and parent.id=:p2 and site.id=:p3 order by site.id, sort",
    // new Parameter(Category.DEL_FLAG_NORMAL, parentId, siteId));
    // }

    // public List<Category> findByIdIn(String[] ids);
    // {
    // return find("from Category where id in (:p1)", new Parameter(new
    // Object[]{ids}));
    // }
