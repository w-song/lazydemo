			...
			<c:when test="${param.a eq '1'}">
				<table>
					<tr><td>包含以下<strong>任意一个</strong>关键词</td><td><input type="text" name="q" value="${q}" class="txt"/>
						<input type="submit" value="搜  索" class="btn btn-default" onclick="$('#a').val('1')"/>
						<input type="submit" value="简单搜索" class="btn btn-default" onclick="$('#a').val('0')"/></td></tr>
				...
				<input type="submit" value="搜  索" class="btn btn-default" onclick="$('#a').val('0')"/>
				<input type="submit" value="高级搜索" class="btn btn-default" onclick="$('#a').val('1')"/>
			</c:otherwise>
		</c:choose>
	</form:form>
	<dl class="search">
